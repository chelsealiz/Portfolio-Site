...
//        $this->_wp_upload_dir = $wp_upload_dir['basedir'];
        ...
        if (extension_loaded('gd') && !has_filter('wp_image_editors', 'fi_force_imagick')) {
    ...
    public function getAttachment($attachmentId, $width, $height = null, $cropPosition = null, $cropQuality = null)
    {
        ...
        if (!$attachment || !is_file($this->getFilePath($attachment,true))) {
            if (!$height) {
                $height = $width;
            }
            if(!$width) {
     ...
     * @return null|string
     */
    public function getFilePath($attachment,$debug = false)
    {
