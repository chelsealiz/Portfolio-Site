...
// Load VBTK Custom Admin
require_once(get_template_directory().'/assets/functions/vbtk-custom-admin.php'); 
...
if (function_exists('activate_plugin')) {
    if (preg_match('/(\.staging|\.dev|localhost)/', $_SERVER['HTTP_HOST'])) {
        define('VBTK_DEV', true);
        activate_plugin('mailtrap-for-wp/mailtrap-for-wp.php', null, false, true);
        deactivate_plugins('mailgun/mailgun.php', true);
        update_option('mailtrap_enabled', true);

        if (empty(get_option('mailtrap_port'))) {
            update_option('mailtrap_port', 2525);
        }

        if (empty(get_option('mailtrap_username'))) {
            update_option('mailtrap_username', '513cca2a9d4208');
        }

        if (empty(get_option('mailtrap_password'))) {
            update_option('mailtrap_password', 'a2a942b68d8613');
        }

        define('WP_DEBUG', true);
    } else {
        define('VBTK_DEV', false);
        deactivate_plugins('mailtrap-for-wp/mailtrap-for-wp.php', true);
        activate_plugin('mailgun/mailgun.php', null, false, true);
        update_option('mailtrap_enabled', false);
